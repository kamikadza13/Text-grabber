...
def make_path_to_folders(path: str, Language):
    if file_exists((path + "/" if path != "" else "") + "Defs"):
        path_to_Def.append(path.replace("\\", "/"))
        # print("\t\tВ " + path + " cуществует Defs")
        # print(path_to_Def)
        os.makedirs((os.path.normpath(
            ("_Translation/" + (path + "/" if path != "" else "") + "Languages/" + Language + "/DefInjected"))).replace(
            "\\", "/"), exist_ok=True)
    ...
    if file_exists((path + "/" if path != "" else "") + "Languages/English/Keyed"):
        ...
        if glob((path + "/" if path != "" else "") + 'Languages/*/Keyed'):
            copytree(str(glob((path + "/" if path != "" else "") + 'Languages/*/Keyed')[0]).replace("\\", "/"),
        ...
        for path, subdirs, files in (os.walk(p_ + "/Defs") if p_ != "" else os.walk(p_ + "Defs")):
            ...
            for file in files:
                if file.endswith('.xml'):
                    ...
                    files_to_translate_new_path.append(("_Translation/" + (p_ + "/" if p_ != "" else "") + "Languages/" + Language + "/" + "DefInjected"))
...
def main(Language):
    ...
    def loadfolder():
        ...
        shcopy("loadFolders.xml", r"_Translation/loadFolders.xml")
