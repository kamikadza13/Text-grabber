...
if file_exists('Settings.txt'):
    with open('Settings.txt', 'r', encoding="utf-8") as settings:
        for c in settings:
            if c.find("Language") != -1:
                Language = c.partition("Language")[2].strip(' =').strip('\n')
            if c.find("Autor") != -1:
                Autor = c.partition("Autor")[2].strip(' =').strip('\n')
            if c.find("Name_add") != -1:
                Name_add = c.partition("Name_add")[2].strip(' =').strip('\n')
            if c.find("Description") != -1:
                Description = c.partition("Description")[2].strip(' =').strip('\n')
if Language == "":
    Language = 'Russian'
if Autor == "":
    Autor = 'Kamikadza13'
...
def find_new_path_for_li(child):
...
shutil.copy(s1_path, d_path)
tree = ET.parse("About/About.xml")
root = tree.getroot()
# supportedVersions = root.find("supportedVersions")
...
# supportedVersions = html.escape(supportedVersions, quote=False)
name = root.find("name").text
packageId = root.find("packageId").text
with open('_Translation/About/About.xml', 'w', encoding="utf-8") as about:
    about.write('<?xml version="1.0" encoding="utf-8"?>' + "\n")
    about.write('<ModMetaData>' + "\n")
    about.write("\t" + '<name>' + name + " " + Name_add + '</name>' + "\n")
    about.write("\t" + '<author>' + Autor + '</author>' + "\n")
    about.write("\t" + '<packageId>' + Autor + "." + packageId + '</packageId>' + "\n")
    about.write("\t" + '<supportedVersions>' + supportedVersions + '</supportedVersions>' + "\n")
    about.write("\t" + '<loadAfter>\n\t\t<li>' + packageId + '</li>\n\t</loadAfter>' + "\n")
